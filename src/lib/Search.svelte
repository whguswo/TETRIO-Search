<script lang="ts">
    ...
    async function search(username: string) {
        ...
        info.user.rating = `${Math.floor(leagueInfo.rating).toString()}점`;
        replays = streams.data.records;
        state.update((v) => {
            v.loader = false;
            return v;
        ...
        recordInfo.total_lose = `${
            leagueInfo.gamesplayed - leagueInfo.gameswon
        }패`;
        recordInfo.win_rate = `(승률: ${
            Math.floor((leagueInfo.gameswon / leagueInfo.gamesplayed) * 10000) /
            100
        }%)`;
...
<div id="search">
    ...
    <input
        ...
        maxlength={16}
        pattern="[0-9a-zA-Z\\_\\-]*"
        on:keyup={usernameKeyupEvent}
        type="text"
        placeholder="유저명(소문자로 입력해주세요.)"
    />
    ...
    {#if info.user.image}
        ...
        <div id="rating">{info.user.rating}</div>
    {/if}
    {#if record}
        <!-- 전적 전적 -->
        <Record {...recordInfo} />
